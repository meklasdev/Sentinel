...
public class IpAnalyzer implements Runnable {
    ...
    /**
     * Gets a location score for the given IP address.
     * Lower scores indicate more suspicious locations.
     * @param ip The IP address to analyze
     * @return A score between 0.0 (suspicious) and 1.0 (trusted)
     */
    public double getLocationScore(String ip) {
        // Default score
        double score = 0.8; // Start with a moderately trusted score
        
        try {
            InetAddress address = InetAddress.getByName(ip);
            IpAnalysis analysis = new IpAnalysis(null); // We don't need a player for basic IP checks
            analysis.setIp(ip);
            analysis.checkVpnProxy();
            
            // Adjust score based on VPN/Proxy detection
            if (analysis.getVpnDetected() || analysis.getProxyDetected()) {
                score -= 0.3; // Significant penalty for VPN/Proxy
            }
            
            // Check if IP is in suspicious ranges (if implemented)
            if (suspiciousRanges.containsKey(ip.substring(0, ip.lastIndexOf('.')))) {
                score -= 0.2;
            }
            ...
            return 0.5; // Neutral score for invalid IPs
        }
    }
}
