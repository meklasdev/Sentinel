...
public class HistoryLogger {
    ...
    public void logSuspiciousActivity(Player player, String activity, String details) {
        String logMessage = String.format(
            "[SUSPICIOUS] Player: %s, Activity: %s, Details: %s",
            player.getName(), activity, details
        );
        
        logger.log(Level.SEVERE, logMessage);
        incrementCounter("suspicious");
        
        // Send Discord alert
        DiscordAlertManager.getInstance().sendAlert("suspicious", logMessage);
    }
    ...
    private void incrementCounter(String activity) {
        ...
        if (count >= threshold) {
            ...
            DiscordAlertManager.getInstance().sendAlert("threshold", alertMessage);
        ...
        for (Map.Entry<String, List<String>> entry : suspiciousPatterns.entrySet()) {
            ...
            if (message.toLowerCase().contains(pattern.toLowerCase())) {
                ...
                if (count >= threshold) {
                    ...
                    DiscordAlertManager.getInstance().sendAlert("suspicious", alertMessage);
